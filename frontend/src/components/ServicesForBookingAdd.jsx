...
export default function ServicesForBookingAdd({ isOpen, onClose, onAdd }) {
  ...
  const fetchServiceDatas = async () => {
    ...
    // Filter services to only include those with forEachRoom = true
    const roomServices = data.services.filter(
      (service) => service.forEachRoom === true
    );
    return roomServices;
  };
    ...
    <div className="fixed inset-0 z-[1100] flex items-center justify-center">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-lg font-bold">Thêm dịch vụ cho phòng</h1>
          <button
            type="button"
        ...
        {/* Thêm các nút xác nhận/thêm dịch vụ ở đây nếu cần */}
        <button
          type="button"
